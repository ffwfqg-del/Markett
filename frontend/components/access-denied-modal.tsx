...
export function AccessDeniedModal() {
  ...
  const handleAuthorize = () => {
    closeAccessDenied()
    // Small delay to ensure the modal is fully unmounted before opening the next one
    setTimeout(() => {
  ...
  return (
    <div 
      className={`fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm transition-opacity duration-200 ${
        isAccessDeniedOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'
      }`}
      style={{ display: isAccessDeniedOpen ? 'flex' : 'none' }}
    >
